  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          // Ambient orb
          Positioned(top: -100, right: -100, child: Container(width: 300, height: 300, decoration: BoxDecoration(shape: BoxShape.circle, gradient: RadialGradient(colors: [AppColors.accent.withValues(alpha: 0.08), Colors.transparent])))),
          ...
          children: [
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Column(
                      ...
                      children: [
                        Text(chat.peerUsername!, style: AppTypography.ui(fontSize: 14, fontWeight: FontWeight.w600, color: Colors.white)),
                        Text(
            ...
            if (chat.mode == 'live') ...[
              ...
              if (_confirmingBlock) ...[
                ...
                IconButton(
                  icon: Text('⛔', style: TextStyle(fontSize: 15, color: _blocked ? AppColors.graphite : AppColors.slate)),
    ...
    if (chat.mode == 'expired') {
      return Center(
        child: Column(mainAxisSize: MainAxisSize.min, children: [
          Text('404', style: AppTypography.display(fontSize: 48, color: Colors.white.withValues(alpha: 0.1))),
          const SizedBox(height: 8),
          Text('Conversation no longer available.', style: AppTypography.ui(fontSize: 14, fontWeight: FontWeight.w600, color: AppColors.fog)),
    ...
    return Center(
      child: Container(
        ...
        child: Row(mainAxisSize: MainAxisSize.min, children: [
          Text(label, style: AppTypography.body(fontSize: 11, color: AppColors.fog)),
          const SizedBox(width: 8),
          Text(_formatTime(marker.ts), style: AppTypography.body(fontSize: 11, color: AppColors.graphite)),
        ]),
      ),
    );
  }
      ...
      child: Column(
        ...
        children: [
          ...
          Container(
            ...
            padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 10),
            decoration: BoxDecoration(
              color: isMe ? AppColors.charcoal : Colors.white.withValues(alpha: 0.08),
              ...
              border: Border.all(color: Colors.white.withValues(alpha: isMe ? 0.08 : 0.1)),
            ),
            child: Text(msg.text, style: AppTypography.body(fontSize: 14, color: isMe ? Colors.white : AppColors.fog)),
          ),
          Padding(
            padding: const EdgeInsets.only(top: 3, left: 4, right: 4, bottom: 6),
            child: Text(_formatTime(msg.ts), style: AppTypography.body(fontSize: 10, color: AppColors.graphite)),
          ),
        ],
      ),
    );
      ...
      child: SafeArea(
        ...
        child: Column(
          children: [
              ...
              children: [
                Expanded(
                  child: TextField(
                    ...
                    minLines: 1,
                    enabled: !disabled,
                    style: AppTypography.body(fontSize: 14, color: Colors.white),
                    decoration: InputDecoration(
                      ...
                      border: OutlineInputBorder(borderRadius: AppRadii.mdAll, borderSide: BorderSide(color: Colors.white.withValues(alpha: 0.08))),
                      enabledBorder: OutlineInputBorder(borderRadius: AppRadii.mdAll, borderSide: BorderSide(color: Colors.white.withValues(alpha: 0.08))),
            ...
            Text('Enter to send · Leave any time', style: AppTypography.body(fontSize: 10, color: AppColors.graphite)),
          ],
        ),
      ),
    );
